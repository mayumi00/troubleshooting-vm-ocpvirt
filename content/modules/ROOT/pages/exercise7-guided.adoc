...
[start=4]
4. 仮想マシン `exercise7` をクリックし、`Snapshot` タブを選択します。
...
[start=4]
4. 仮想マシンのスナップショットを復元します。ユーザーのホームディレクトリに `exercise7-restoresnapshot.yaml` ファイルがあります。確認してください。
...
[start=6]
6. VirtualMachineRestore が作成されていることを確認する
...
この演習では、仮想マシンのスナップショットを復元するには、仮想マシンをシャットダウンする必要があることを学びました。KubeVirtはメモリ スナップショットをサポートしていないためです。