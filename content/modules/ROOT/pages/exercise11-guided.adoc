...
[NOTE]
====
* あなたのタスクは、仮想マシン間の通信を妨げている問題を特定し、トラブルシューティングを行うことです。問題を解決して接続を復旧させてください。
* 必要に応じてを再作成できます
...
[start=1]
1. Virtualization - VirtualMachines に移動し、のIPアドレスを確認します。
...
[start=4]
4. 仮想マシン `exercise11-a` をクリックします。コンソールを開き、ユーザー名 `lab-user` 、パスワード `redhat1` でログインしてください。
...
[start=5]
5.  `exercise11-a` で、 `Configuration` → `Initial run` をクリックします。
...
[start=8]
8.  `exercise11-b` に対しても同じ手順を実行し、このIPアドレスが/32ネットマスクを使用して誤って設定されていることを確認します。この設定を修正する必要があります。
...
[start=9]
9. YAMLタブをクリックし、YAMLファイルをダウンロードしてください。このファイルは、正しいネットマスクを使用してを再作成するために使用されます。
...
[start=10]
10. `Actions` -> `Stop` で仮想マシンを停止し、次に `Actions` -> `Delete` をクリックして、仮想マシン `exercise11-b` を削除します。
